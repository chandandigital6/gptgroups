<x-layouts::app :title="__('Strategy Sections')">
    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="text-sm text-neutral-500 dark:text-neutral-400">
                    Manage growth strategy section content and strategy cards.
                </p>
            </div>

            <a href="{{ route('strategy-sections.create') }}"
        ...
        <div class="grid auto-rows-min gap-4 md:grid-cols-4">
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ \App\Models\StrategySection::count() }}
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ \App\Models\StrategySection::where('status', 1)->count() }}
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ \App\Models\StrategySection::where('status', 0)->count() }}
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ optional(\App\Models\StrategySection::latest()->first())->id ?? 0 }}
                </div>
            </div>
        </div>

        <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
            <form action="{{ route('strategy-sections.index') }}" method="GET" class="flex flex-col gap-3 md:flex-row md:items-center">
                <input type="text"
                       name="search"
                       value="{{ request('search') }}"
                       placeholder="Search by title, label, description or strategy..."
        ...
        <div class="relative h-full flex-1 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <table class="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                    <thead class="bg-neutral-100 dark:bg-neutral-800">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-neutral-600 dark:text-neutral-300">Strategies</th>
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                        @forelse($strategySections as $strategySection)
                            <tr class="transition hover:bg-neutral-50 dark:hover:bg-neutral-800/60">
                                ...
                                <td class="px-6 py-4">
                                    <div>
                                        ...
                                        <div class="mt-1 text-xs text-blue-600 dark:text-blue-300">
                                            {{ $strategySection->label ?: 'No Label' }}
                                        </div>
                                        ...
                                        @foreach([1, 2, 3, 4] as $i)
                                            ...
                                            @if($strategySection->{$titleField})
                                                <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
                                                    {{ $strategySection->{$numberField} }} {{ $strategySection->{$titleField} }}
                                                </span>
                                            @endif
                                        @endforeach
                                    </div>
                                ...
                                <td class="px-6 py-4 text-sm font-semibold text-neutral-800 dark:text-neutral-200">
                                    {{ $strategySection->sort_order }}
                                </td>

                                <td class="px-6 py-4">
                                    @if($strategySection->status)
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-3">
                                        <a href="{{ route('strategy-sections.show', $strategySection) }}"
                                        ...
                                        <form action="{{ route('strategy-sections.destroy', $strategySection) }}"
                                              method="POST"
                                              onsubmit="return confirm('Delete this strategy section?')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-20 text-center">
                                    <h3 class="text-lg font-semibold text-neutral-800 dark:text-white">
                                        No Strategy Section Found
                                    </h3>

                                    <p class="mt-1 text-sm text-neutral-500">
                                        Create your first strategy section.
                                    </p>
                                </td>
                            </tr>
                        @endforelse
            ...
            @if(method_exists($strategySections, 'links'))
                <div class="border-t border-neutral-200 px-6 py-4 dark:border-neutral-700">
                    {{ $strategySections->links() }}
                </div>
            @endif
        </div>
    </div>
