<x-layouts::app :title="__('Retail Outlet Sections')">
    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                <h1 class="text-2xl font-bold text-neutral-900 dark:text-white">
                    Retail Outlet Sections
                </h1>

                <p class="text-sm text-neutral-500 dark:text-neutral-400">
                    Manage retail outlet section content, cards, button and images.
                </p>
            </div>

            <a href="{{ route('retail-outlet-sections.create') }}"
        ...
        <div class="grid auto-rows-min gap-4 md:grid-cols-4">
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ \App\Models\RetailOutletSection::count() }}
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ \App\Models\RetailOutletSection::where('status', 1)->count() }}
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ \App\Models\RetailOutletSection::where('status', 0)->count() }}
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ optional(\App\Models\RetailOutletSection::latest()->first())->id ?? 0 }}
                </div>
            </div>
        </div>

        <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
            <form action="{{ route('retail-outlet-sections.index') }}" method="GET" class="flex flex-col gap-3 md:flex-row md:items-center">
                <input type="text"
                       name="search"
        ...
        <div class="relative h-full flex-1 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                        @forelse($retailOutletSections as $retailOutletSection)
                            <tr class="transition hover:bg-neutral-50 dark:hover:bg-neutral-800/60">
                                ...
                                <td class="px-6 py-4">
                                    <div>
                                        ...
                                        <div class="mt-1 text-xs text-blue-600 dark:text-blue-300">
                                            {{ $retailOutletSection->label ?: 'No Label' }}
                                        </div>
                                    ...
                                    <div class="flex -space-x-3">
                                        @foreach(['image_1', 'image_2', 'image_3', 'image_4'] as $imageField)
                                            @if($retailOutletSection->{$imageField})
                                                <img src="{{ asset('storage/' . $retailOutletSection->{$imageField}) }}"
                                                     class="h-12 w-12 rounded-xl border-2 border-white object-cover shadow-sm dark:border-neutral-900"
                                                     alt="Image">
                                            @endif
                                        @endforeach

                                        @if(!$retailOutletSection->image_1 && !$retailOutletSection->image_2 && !$retailOutletSection->image_3 && !$retailOutletSection->image_4)
                                ...
                                <td class="px-6 py-4 text-sm font-semibold text-neutral-800 dark:text-neutral-200">
                                    {{ $retailOutletSection->sort_order }}
                                </td>

                                <td class="px-6 py-4">
                                    @if($retailOutletSection->status)
                                ...
                                <td class="whitespace-nowrap px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-3">
                                        <a href="{{ route('retail-outlet-sections.show', $retailOutletSection) }}"
                                        ...
                                        <form action="{{ route('retail-outlet-sections.destroy', $retailOutletSection) }}"
                                              method="POST"
                                              onsubmit="return confirm('Delete this retail outlet section?')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-20 text-center">
                                    <h3 class="text-lg font-semibold text-neutral-800 dark:text-white">
                                        No Retail Outlet Section Found
                                    </h3>

                                    <p class="mt-1 text-sm text-neutral-500">
                                        Create your first retail outlet section.
                                    </p>
                                </td>
                            </tr>
                        @endforelse
            ...
            @if(method_exists($retailOutletSections, 'links'))
                <div class="border-t border-neutral-200 px-6 py-4 dark:border-neutral-700">
                    {{ $retailOutletSections->links() }}
                </div>
            @endif
        </div>
    </div>
