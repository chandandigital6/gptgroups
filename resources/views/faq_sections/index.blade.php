<x-layouts::app :title="__('FAQ Sections')">
    ...
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="text-sm text-neutral-500 dark:text-neutral-400">
                    Manage page-wise FAQ sections for home, about, brands, services and custom pages.
                </p>
            </div>

            <a href="{{ route('faq-sections.create') }}"
        ...
        <div class="grid auto-rows-min gap-4 md:grid-cols-4">
            ...
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                <div class="text-sm text-neutral-500 dark:text-neutral-400">FAQ Items</div>
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ $stats['items'] ?? 0 }}
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                <div class="text-sm text-neutral-500 dark:text-neutral-400">Pages</div>
                <div class="mt-2 text-3xl font-bold text-black dark:text-white">
                    {{ $stats['pages'] ?? 0 }}
                </div>
            </div>
        ...
        <div class="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
            <form action="{{ route('faq-sections.index') }}" method="GET" class="grid gap-3 md:grid-cols-4">
                <input type="text"
                       name="search"
                       value="{{ request('search') }}"
                       placeholder="Search FAQ..."
                       class="md:col-span-2 w-full rounded-xl border border-neutral-200 bg-white px-4 py-3 text-sm text-neutral-900 outline-none transition focus:border-black dark:border-neutral-700 dark:bg-neutral-950 dark:text-white dark:focus:border-white">
                ...
                <select name="page_slug"
                    ...
                    @foreach($pages as $value => $label)
                        <option value="{{ $value }}" @selected(request('page_slug') === $value)>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="relative h-full flex-1 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
            <div class="overflow-x-auto">
                <table class="min-w-[1150px] w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                    ...
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-neutral-600 dark:text-neutral-300">FAQ Section</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-neutral-600 dark:text-neutral-300">Page</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-neutral-600 dark:text-neutral-300">Items</th>
                        ...
                        <th class="min-w-[260px] px-6 py-4 text-right text-xs font-semibold uppercase tracking-wider text-neutral-600 dark:text-neutral-300">Actions</th>
                    </tr>
                    </thead>

                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
                    @forelse($faqSections as $faqSection)
                        <tr class="transition hover:bg-neutral-50 dark:hover:bg-neutral-800/60">
                            ...
                            <td class="min-w-[260px] whitespace-nowrap px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('faq-sections.show', $faqSection->id) }}"
                                    ...
                                    <form action="{{ route('faq-sections.destroy', $faqSection->id) }}"
                                          method="POST"
                                          onsubmit="return confirm('Delete this FAQ section?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-20 text-center">
                                ...
                                <p class="mt-1 text-sm text-neutral-500">
                                    Create your first FAQ section.
                                </p>

                                <a href="{{ route('faq-sections.create') }}"
                                   class="mt-4 inline-flex rounded-xl bg-black px-5 py-3 text-sm font-semibold text-white">
                                    Create FAQ Section
                                </a>
                            </td>
                        </tr>
